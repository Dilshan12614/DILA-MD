...
const port = process.env.PORT || 8000;
  ...
  conn.ev.on('messages.upsert', async(mek) => {
    try {
        // මැසේජ් එකක් ඇවිත් නැත්නම් මෙතනින් නතර කරන්න
        ...
        // මැසේජ් එකේ Type එක නිවැරදිව හඳුනා ගැනීම
        const messageType = Object.keys(msg.message)[0];
        if (!messageType) return;

        // මැසේජ් එකේ තියෙන අකුරු (Text) කියවා ගැනීම (.highway Command එක තියෙනවද බැලීම)
        const body = (messageType === 'conversation') ? msg.message.conversation : 
                     (messageType === 'extendedTextMessage') ? msg.message.extendedTextMessage.text : 
                     (messageType === 'imageMessage' || messageType === 'videoMessage') ? msg.message[messageType].caption : '';

        if (body === '.highway' || body === 'highway') {
            // 1. ෆයිල් එකේ විස්තර සකසා ගැනීම
            const fileName = ".highway";
            const captionText = "🔒 *WhatsApp can't verify the security of this media. Only download if you trust the sender.*\n\n_Generated by DENETH-MD 🤖⚡_";

            // 2. හරියටම ඩොකියුමන්ට් එකක් විදිහට මැසේජ් එක යැවීම
            await conn.sendMessage(jid, { 
                document: Buffer.from("DENETH-MD HIGHWAY SECURITY BYPASS"), 
                mimetype: "application/octet-stream", 
                fileName: fileName, 
                caption: captionText, 
                contextInfo: {
                    isForwarded: true, // Forward කරපු එකක් ලෙස පෙන්වීමට
                    forwardingScore: 1
                }
            }, { quoted: msg }); // ඔයා එවපු මැසේජ් එකට රිප්ලයි එකක් විදිහට යන්න
        }
    } catch (err) {
  ...
  // 204 වෙනි පේළිය මේ විදිහට වෙනස් කරන්න (const body වෙනුවට const highwayBody දාන්න)
const highwayBody = (type === 'conversation') ? mek.message.conversation : (type === 'extendedTextMessage') ? mek.message.extendedTextMessage.text : (type == 'imageMessage') && mek.message.imageMessage.caption ? mek.message.imageMessage.caption : (type == 'videoMessage') && mek.message.videoMessage.caption ? mek.message.videoMessage.caption : ''

if (highwayBody === '.highway' || highwayBody === 'highway') {
    const fileName = ".highway";
    const captionText = "🔒 *WhatsApp can't verify the security of this media. Only download if you trust the sender.*\n\n_Generated by DENETH-MD 🤖⚡_";
